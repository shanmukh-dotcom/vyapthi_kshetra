...
for (const line of lines) {
  ...
  if (line.includes('src\\\\style.css') && line.includes('role-card')) {
    try {
      const obj = JSON.parse(line);
      const tc = obj.tool_calls.find(t => t.name === 'write_to_file' && t.args.TargetFile.endsWith('style.css'));
      if (tc) {
        fs.writeFileSync('original-style-role.css', tc.args.CodeContent);
        console.log("Extracted role style.css successfully!");
        break;
      }
    } catch(e) {}
  }
