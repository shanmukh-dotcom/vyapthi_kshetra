...
for (const line of lines) {
  ...
  if (line.includes('src\\\\style.css') && line.includes('language-card')) {
    try {
      const obj = JSON.parse(line);
      const tc = obj.tool_calls.find(t => t.name === 'write_to_file' && t.args.TargetFile.endsWith('style.css'));
      if (tc) {
        fs.writeFileSync('original-style.css', tc.args.CodeContent);
        console.log("Extracted original style.css successfully!");
        break;
      }
    } catch(e) {}
  }
